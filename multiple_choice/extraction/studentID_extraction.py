...
class StudentIDExtract():
    def __init__(self, config):
        self.config = config

    def __call__(self, img):
        t = self.config['SID_TOP'] 
        l = self.config['SID_LEFT']
        b = self.config['SID_BOT'] 
        r = self.config['SID_RIGHT']
        ...
        img = img[t:b, l:r]
        if self.config['SID_VISUALIZE']:
            cv2.imwrite(DEBUG_DIR + f"0_student_id.jpg", img)
        ...
        # 2 hang o tren duoi student id
        boxes_cnts = sorted(contours, key=cv2.contourArea, reverse=True)[:2]
        boxes_img = cv2.drawContours(img.copy(), boxes_cnts, -1, (0, 255, 255), 1)
        if self.config['SID_VISUALIZE']:
        ...
        boxes_cnts = sorted(contours, key=cv2.contourArea, reverse=True)[:6]
        boxes_img = cv2.drawContours(img.copy(), boxes_cnts, -1, (0, 255, 255), 1)
        if self.config['SID_VISUALIZE']:
            cv2.imwrite(DEBUG_DIR + "6_boxes.jpg", boxes_img)
        ...
        studentid = [int(int(box[1] + box[3] / 2) * 10 /
                        binary_image.shape[0]) for box in boxes]
        return "".join(map(str, studentid))